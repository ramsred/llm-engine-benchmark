...
def generate_report(results_dir: str | Path) -> dict[str, Path]:
    ...
    provenance = _build_provenance(root, runs, summaries)
    atomic_write_json(report_dir / "provenance.json", provenance)
    source_fields = [
        "engine",
        "cache_mode",
        "concurrency",
...
def _build_provenance(
    root: Path, runs: Sequence[Mapping[str, Any]], summaries: Sequence[Mapping[str, Any]]
) -> dict[str, Any]:
    """Record the evidence identity used to generate public report tables."""
    ...
    try:
        ...
        commit = "unknown"
    lock_candidates = [
        root / "environment" / "experiment_lock.json",
        root.parent / "experiment.lock.json",
    ]
    lock_path = next((path for path in lock_candidates if path.exists()), None)
    return {
        "generated_by": "llm-engine-benchmark",
        "generated_at": utc_now(),
        "benchmark_commit": commit,
        "experiment_lock": str(lock_path) if lock_path else None,
        "experiment_lock_sha256": sha256_file(lock_path) if lock_path else None,
        "accepted_runs": sum(1 for row in runs if row["valid"]),
        "accepted_repetitions": sorted({int(row["repetition"]) for row in runs if row["valid"]}),
...
def _render_markdown(
    ...
    lines = [
        ...
        "",
        "<!-- Generated by llm-engine-benchmark. Do not edit manually. -->",
        "",
        f"Generated: {utc_now()}",
        "",
